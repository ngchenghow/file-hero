...
v0.1.2 支持相册/文件管理器的系统分享：选中一个或多个文件 → 分享 → File Hero。首次选择 SSD 目标目录，之后自动使用仍可访问的上次位置；填写批次名称和描述后点「存入」。可在分享存入页更换 SSD。App 未启动或已经打开时都支持接收文件分享；普通文字/网址分享不会当作文件写入。App 使用专用 File Hero 图标。
