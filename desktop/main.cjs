...
function core(command, relative = '', argument) {
  ...
  return new Promise((resolve, reject) => execFile(exe, [command, root, relative, ...(argument === undefined ? [] : Array.isArray(argument) ? argument : [argument])], { windowsHide: true, maxBuffer: 32 * 1024 * 1024, timeout: 0 }, (error, stdout, stderr) => {
  ...
  ipcMain.handle('hero', async (event, action, relative = '', value = '') => {
    ...
    try {
      ...
      if (action === 'import') {
        ...
        if (typeof batch.name !== 'string' || typeof batch.description !== 'string') throw new Error('批次名称和描述无效');
        const result = await dialog.showOpenDialog(window, { title: '选择本批次文件（可多选）', properties: ['openFile', 'multiSelections'] });
        if (result.canceled || !result.filePaths.length) return null;
        return await core('batch', relative, [batch.name, batch.description, ...result.filePaths]);
      }
      if (action === 'export') {
        const result = await dialog.showSaveDialog(window, { title: '导出副本（请选择新文件名）', defaultPath: path.basename(relative) });
        return result.canceled ? null : await core('export', relative, result.filePath);
      }
